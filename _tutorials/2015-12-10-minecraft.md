...
This tutorial describes running a Minecraft server on Carina so that you can explore the world of Minecraft with your friends and family.
...
Minecraft is a multi-player game that let's you explore a vast 3D world, craft items from world's bountiful resources, and survive nights filled with monstrous mobs.

### Create a data volume container

1. Create a data volume container (DVC) to store your Minecraft world data in a persistent volume that's easy to access from other containers.
    ...
    **Note**: Take extreme care not to remove this container. If this container get's removed you will lose all of Minecraft world data.  
...
1. Run a Minecraft server.
    ...
    This container is connected to the DVC using the `--volumes-from` flag. It uses the `--publish` flag to expose the server on the standard Minecraft port. Many `--env` flags are used to configure Minecraft server settings. You'll need to change the `OPS` and `WHITELIST` lists to include your own username. A full description of all of the settings can be found in the [itzg/minecraft-server](https://hub.docker.com/r/itzg/minecraft-server/) Docker Hub page.
...
1. Inspect a single file.
    ...
    The output of this `docker run` command is the contents of the `/data/whitelist.json` file, which is the whilelist of all of the users allowed to login to your server.
...
1. Check the version of Minecraft.
    ...
    The output of this `docker logs` command is the lone log line with the version of Minecraft your server is running.
...
1. Attach to the container.
    ...
    There is no output from this `docker attach` command. That's because the server is waiting for input! To run commands on the server, simply type in the commands as below.

    When you've finished running commands, you need to hit Ctrl-p and then Ctrl-q to detach from the container. Do not use Ctrl-c! That will kill your server and you'll restart it by going through [Run a Minecraft server](#run-a-minecraft-server) again
...
#### Get help
    ...
    The output of this `/help` command give you all of the commands you can possibly run on the server. You can jump to different pages by running the command like `/help 2`.
...
#### Whitelist a player
    ...
    The output of this `/whitelist add` command shows you the player just whitelisted.
    ...
    **Note**: It's important to know how to add users to the whitelist so others can join your server.
...
#### Op a player
    ...
    The output of this `/whitelist add` command shows you the player just opped. That player can now run all operator level commands in the game.
...
1. Discover the IP address where the server is running.
    ...
    The output of this `docker port` command is the ip:port that you need to connect to the Minecraft server.

1. Connect to the Minecraft server.

    1. Open the Minecraft game.
    1. Login using your username from the `WHITELIST` above.
    1. On the home screen click Multiplayer
    1. Click Add server
      1. Server Name: Minecraft Server on Carina
      1. Server Address: [copy/paste the ip:port from the above step]
      1. Click Done
    ...
    When you want others to join your Minecraft server, give them the ip:port and use their username in [Whitelist a player](#whitelist-a-player).

### Upgrade to the next version of Minecraft

New versions of Minecraft are consistently being released. Because you created a DVC to store your world data, it's easy to upgrade to the next version of Minecraft. You can even upgrade to the latest snapshot version.
...
1. Write any server changes to disk.
    ...
    Hit Ctrl-p and then Ctrl-q to detach from the container.

1. Kill the current Minecraft server
    ...
    The output of this `docker rm` command is the name of the container your just removed.
...
1. Check the version of Minecraft.
    ...
    The output of this `docker logs` command is the lone log line with the version of Minecraft your new server is running.
