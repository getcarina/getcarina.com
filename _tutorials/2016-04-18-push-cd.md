...
10. Wait for the first build to complete (a few minutes). You can see the
    build status by going to the Build Details page. You will see a green
    "Success" status when it has finished.
...
Copy the `WEBHOOK_URL` value to your clipboard.
...
request did not satisfy the following requirements: that it is a `POST` request with well-formed
JSON payload ([documented here](https://docs.docker.com/docker-hub/webhooks/)), and that it
contains token query parameter in its URL which matches the `TOKEN` environment variable
specified when wiretap was first run.

You can also `curl` your webhook and see what happens:
